...
namespace PCSC {
  ...
  int init_context( SCARDCONTEXT &hContext ) {
    ...
    std::cout << "Init Context: Ok" << std::endl;
    return CONTEXT_SET;
  }
  ...
  int update_readers( SCARDCONTEXT &hContext, SCARD_READERSTATE *&readersState, v8::Persistent<v8::Object> &readers ){
    std::cout << "Update Readers" << std::endl;
    LONG rv;
    DWORD dwReaders;
    ...
    std::cout << "Update Readers 1" << std::endl;
    ...
    std::cout << "Update Readers 2:" << rv << ", mszR:" << (uint64_t)mszReaders << ", dwR:" << dwReaders << std::endl;
    rv = SCardListReaders(hContext, NULL, (LPTSTR)&mszReaders, &dwReaders);
    std::cout << "Update Readers 2:" << rv << ", mszR:" << mszReaders << ", dwR:" << dwReaders << std::endl;
    ...
    std::cout << "Update Readers 3" << std::endl;
    ...
    std::cout << "Update Readers 4" << std::endl;
    ...
    std::cout << "Update Readers 5" << std::endl;
    ...
    //SCardFreeMemory(hContext, mszReaders);
    #endif
    std::cout << "Readers Updated" << std::endl;
  ...
  int check_card_status( SCARDCONTEXT &hContext, SCARD_READERSTATE *&readersState, v8::Persistent<v8::Object> &readers, v8::Local<v8::Function> &callback ){
    ...
    std::cout << "Check Status: Ok" << std::endl;
    if ((rv == SCARD_S_SUCCESS) || (static_cast<unsigned int>(rv) == SCARD_E_TIMEOUT)) {
      std::cout << "Check Status: Success" << std::endl;
      ...
      for (i=0; i<nbReaders; i++) {
        ...
        if(evt & SCARD_E_SHARING_VIOLATION) {
